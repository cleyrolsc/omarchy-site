...
for (const [name, viewport] of Object.entries(viewports)) {
  for (const route of Object.keys(reference.viewports.desktop)) {
    test(`redesign layout ${name} ${route}`, async ({ page }) => {
      ...
      assertLayout(actual, expected);
      if (route === "/meetups/") {
        await page.getByRole("button", { name: /^Europe/ }).click();
        assertLayout(await metrics(page), reference.states[name].meetupsEurope);
      }
    });
  }
}

function assertLayout(actual: any, expected: any) {
  expect(actual.overflow).toBe(false);
  expect(Math.abs(actual.main.h - expected.main.h), "main height").toBeLessThan(
    4,
  );
  expect(
